...
<template>
	<table class="w-full border-collapse table-fixed border-spacing-0">
		...
		<tbody>
			<tr v-for="episode in store.channel.episodes" :key="episode.guid" class="channel-item relative h-[60px] cursor-default text-[15px] hover:bg-gray-700">
				...
				<td class="h-[60px] overflow-hidden px-8 py-4 text-start">
					<div>
						<h2 
							class="w-fit text-xl font-semibold mt-2 hover:underline hover:text-gray-300 cursor-pointer"
							@click="goToEpisode(episode.guid)"
						>
							{{ episode.title }}
						</h2>
						<p class="text-[#757575]">{{ episode.publishDate }}</p>
					</div>
				</td>
